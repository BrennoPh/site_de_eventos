...
public class CEventoController {
    ...
    public String index(Model model, HttpSession session, @RequestParam(name = "q", required = false) String query) {
        ...
        // Chama o serviço para buscar os eventos, passando o termo de busca (pode ser nulo).
    ...
    public String criarEvento(@RequestParam String nomeEvento, @RequestParam LocalDateTime dataEvento, @RequestParam String local,
        ...
        if (usuarioLogado instanceof Organizador) {
            ...
            // Delega toda a lógica de criação do evento (incluindo o uso do Builder) para a camada de serviço.
        ...
        // Independentemente de quem fez a requisição, redireciona para a página inicial ao final.
        return "redirect:/";
    }
    ...
    public String exibirDetalhesEvento(@PathVariable("id") int id, Model model) {
        // Busca o evento pelo ID; o retorno é um Optional para tratar o caso de não encontrar.
    ...
    public String cancelarEvento(@PathVariable("id") int eventoId, HttpSession session, RedirectAttributes redirectAttributes) {
        ...
        try {
            // Tenta cancelar o evento através do serviço, passando o ID do evento e o organizador.
            eventoService.cancelarEvento(eventoId, (Organizador) usuarioLogado);
            // Se tiver sucesso, adiciona uma mensagem de sucesso para ser exibida após o redirecionamento.
